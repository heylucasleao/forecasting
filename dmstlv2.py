...
class DMSTLWrapperV2(BaseEstimator, RegressorMixin):
    """
    ...
    def fit(
        ...
        for uid, group in df.groupby(id_col):
            ...
            df_res_uid = group_sorted[
                [self.id_col_, self.time_col_] + self.exog_cols_
            ].copy()
            df_res_uid[self.target_col_] = residual_part
            residual_dfs.append(df_res_uid)
    ...
    def predict(
        ...
        for uid, models in self.fitted_models_.items():
            ...
            uid_resid_mask = df_resid_preds[self.id_col_] == uid
            df_uid_resid = df_resid_preds[uid_resid_mask].copy()

            model_cols = self._get_model_cols(df_uid_resid)

            for col in model_cols:
                res = df_uid_resid[col].values + trend_preds + seasonal_preds
                ...
                df_uid_resid[col] = res

            preds_list.append(df_uid_resid)
